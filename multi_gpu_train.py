...
with tf.device(FLAGS.input_device):
    reshape_size = [FLAGS.reshape_height, FLAGS.reshape_weight]
    inputs_queue = pascal_inputs(
...
def model_fn(inputs_queue):
    name_batch, image_batch, label_batch = inputs_queue
...
# set up optimizer
with tf.device(FLAGS.optimize_device):
...
FLAGS.num_replicas = len(FLAGS.run_device)

replicas = []
for gpu_id in range(FLAGS.num_replicas):
    with tf.name_scope('Tower : {}'.format([gpu_id])) as ns:
        with tf.variable_scope(tf.get_variable_scope(),
                               reuse=False if gpu_id == 0 else True):
            with tf.device('/GPU:{}'.format(FLAGS.train_device[gpu_id])) as ds:
...
#             sess.run([pixel_acc, mean_loss, reg_loss, global_step, model_dp.summary_op, model_dp.train_op])
